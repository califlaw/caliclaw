...
class TestOpenclawMigrator:
    ...
    def openclaw_project(self, tmp_path):
        ...
        # Soul files
        soul_dir = root / "agents" / "global" / "main"
        soul_dir.mkdir(parents=True)
        (soul_dir / "SOUL.md").write_text("# openclaw soul")
        (soul_dir / "IDENTITY.md").write_text("name: openclaw-bot")
        (root / "agents" / "SECURITY.md").write_text("# security rules")

        # Memory as JSON
        mem_dir = root / "memory"
        mem_dir.mkdir()
        memories = [
            {"name": "User Profile", "description": "Info about user", "type": "user", "content": "Likes Python"},
            {"name": "Server Setup", "description": "VPS config", "type": "project", "content": "Ubuntu 22.04"},
        ]
        (mem_dir / "memories.json").write_text(json.dumps(memories))

        # Skills
        skill_dir = root / "skills" / "coding"
        ...
        conn.execute("CREATE TABLE messages (id INTEGER PRIMARY KEY, role TEXT, content TEXT, session_id TEXT, timestamp REAL)")
        conn.execute("INSERT INTO messages VALUES (1, 'user', 'hello', 'sess-1', 1000)")
        conn.execute("INSERT INTO messages VALUES (2, 'assistant', 'hi', 'sess-1', 1001)")
        conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY, agent_name TEXT, status TEXT)")
        conn.execute("INSERT INTO sessions VALUES ('sess-1', 'main', 'active')")
        conn.commit()
        conn.close()

        # Config
        (root / ".env").write_text("TELEGRAM_BOT_TOKEN=old-token\nCLAUDE_DEFAULT_MODEL=haiku\n")
    ...
    def test_discover_components(self, openclaw_project, settings):
        ...
        assert avail[MigrationComponent.SKILLS] is True
        assert avail[MigrationComponent.DB] is True
        assert avail[MigrationComponent.CONFIG] is True

    def test_plan_soul(self, openclaw_project, settings):
        from core.migrators.openclaw import OpenclawMigrator
        m = OpenclawMigrator(openclaw_project, settings=settings)
        ...
        assert len(soul_items) >= 2  # SOUL.md, IDENTITY.md, possibly SECURITY.md
        names = [i.description for i in soul_items]
        assert any("SOUL.md" in n for n in names)

    def test_plan_memory_json(self, openclaw_project, settings):
        from core.migrators.openclaw import OpenclawMigrator
        m = OpenclawMigrator(openclaw_project, settings=settings)
        ...
        assert len(mem_items) >= 2  # 2 JSON entries
        assert any("User Profile" in i.description for i in mem_items)
    ...
    def test_plan_db(self, openclaw_project, settings):
        ...
        assert any("messages" in i.description for i in db_items)

    def test_execute_soul_copy(self, openclaw_project, settings):
        from core.migrators.openclaw import OpenclawMigrator
        m = OpenclawMigrator(openclaw_project, settings=settings)
        ...
        tgt_soul = settings.agents_dir / "global" / "main" / "SOUL.md"
        assert tgt_soul.exists()
        assert "openclaw soul" in tgt_soul.read_text()

    def test_execute_memory_convert(self, openclaw_project, settings):
        from core.migrators.openclaw import OpenclawMigrator
        m = OpenclawMigrator(openclaw_project, settings=settings)
